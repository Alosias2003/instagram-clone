...
// Updated mobile navigation items - replaced Profile with More
const mobileNavItems = [
  { label: 'Home', icon: <HomeIcon />, path: '/home' },
  { label: 'Search', icon: <SearchIcon />, isSearch: true },
  { label: 'Create', icon: <AddBoxOutlinedIcon />, path: '/createpost' },
...
const Navbar = () => {
  ...
  const handleMobileMoreClick = (event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const handleNavigation = (path) => {
    if (path && typeof path === 'string') {
      try {
        navigate(path);
        handleMobileMoreClose(); // Close mobile more menu when navigating
      } catch (error) {
        console.error('Navigation error:', error);
        toast.error('Navigation failed. Please try again.', { position: 'top-center', autoClose: 2000 });
      }
  ...
  const handleMobileNavChange = (event, newValue) => {
    ...
    const item = mobileNavItems[newValue];
    if (item.isSearch) handleSearchToggle();
    else if (item.isMore) handleMobileMoreClick(event);
    else if (item.path) handleNavigation(item.path);
  };
    ...
    <>
          ...
          <Box sx={{ marginTop: 'auto', pb: 2 }}>
            {belowNavItems.map((item) => (
              <IconButton 
                ...
                sx={{ margin: '4px 8px', display: 'block' }}
              >
                {item.icon}
              </IconButton>
            ))}
      ...
      <Popover
        ...
        PaperProps={{
          sx: { 
            ...
            boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
            border: '1px solid #dbdbdb',
            marginBottom: '60px' // Add margin to avoid bottom navigation
          },
        }}
        sx={{
          '& .MuiBackdrop-root': {
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Button 
            sx={{ justifyContent: 'flex-start', textTransform: 'none', py: 1.5, px: 2 }}
            onClick={() => handleNavigation('/meta-ai')}
          ...
          <Button 
            ...
            startIcon={<ThreadsIcon style={{ width: 20, height: 20, fill: '#262626' }} />}
          >
            Threads
          </Button>
