...
import {
  Avatar, Box, Card, CardContent, IconButton,
  Typography, Stack, CircularProgress, useMediaQuery,
  useTheme, TextField, Button, Skeleton
} from '@mui/material';
import {
  Favorite, FavoriteBorder, ChatBubbleOutline, MoreHoriz,
  BookmarkBorder, Send, EmojiEmotions
  ...
  const renderCaption = (post) => {
    ...
    return (
      ...
      >
        ...
        {displayText}
        {shouldTruncate && (
          <Typography component="span" sx={{ color: '#8e8e8e', cursor: 'pointer', ml: 0.5 }}>
            more
          </Typography>
        )}
      </Typography>
  ...
  >
    <Box sx={{ display: 'flex', alignItems: 'center', p: 2 }}>
      ...
      <Box sx={{ ml: 2, flexGrow: 1 }}>
        <Skeleton variant="text" height={10} width="40%" animation="wave" />
      </Box>
    ...
    <CardContent>
      <Skeleton variant="text" height={10} width="60%" animation="wave" />
      <Skeleton variant="text" height={10} width="40%" animation="wave" />
      <Skeleton variant="text" height={10} width="80%" animation="wave" />
    </CardContent>
  </Card>
);
            ...
            >
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2 }}>
                <Stack direction="row" spacing={1.2} alignItems="center">
                  <Avatar
                    src={post.author?.profileImage || post.author?.profilePic || '/default-avatar.png'}
                    sx={{ width: avatarSize, height: avatarSize, border: '1px solid #dbdbdb' }}
                  />
                  <Box>
                    <Typography
                      fontWeight="600"
              ...
              {/* Media */}
              {post.content?.image && (
                <Box sx={{ width: '100%', overflow: 'hidden', bgcolor: '#000' }}>
                  <Box
                    component="img"
                    src={post.content.image}
              ...
              {post.content?.video && (
                <Box sx={{ width: '100%', overflow: 'hidden', bgcolor: '#000' }}>
                  <video src={post.content.video} controls style={{ width: '100%', objectFit: 'contain' }} />
                </Box>
              )}

              {/* Post Actions */}
              <CardContent sx={{ p: 0 }}>
                <Stack direction="row" justifyContent="space-between" sx={{ px: 2, pt: 1 }}>
                  <Stack direction="row" spacing={1}>
                    <IconButton onClick={() => toggleLike(post.id)} size="small" sx={{ p: 0.5 }}>
                      {likedPosts[post.id] ? (
                        <Favorite sx={{ fontSize: iconSize, color: '#ed4956' }} />
                      ) : (
                        <FavoriteBorder sx={{ fontSize: iconSize }} />
                      )}
                    </IconButton>
                ...
                <Typography fontWeight="600" sx={{ px: 2, pb: 0.5, fontSize: '14px' }}>
                  {likesCount[post.id] === 1 ? '1 like' : `${likesCount[post.id]?.toLocaleString() || 0} likes`}
                </Typography>
                ...
                {/* Comments */}
                {comments[post.id]?.slice(-2).map((comment) => (
                  <Typography key={comment.id} sx={{ px: 2, fontSize: '14px' }}>
                    <b>{comment.username}</b> {comment.text}
                  </Typography>
                ))}

                {/* Timestamp */}
                <Typography variant="caption" sx={{ px: 2, color: '#8e8e8e', fontSize: '12px' }}>
                  {formatDistanceToNow(new Date(post.createdAt), { addSuffix: true })}
                </Typography>

                {/* Add Comment */}
                <Box sx={{ borderTop: '1px solid #efefef', px: 2, py: 1, display: 'flex', gap: 1 }}>
                  <IconButton size="small" sx={{ p: 0.5 }}>
                    <EmojiEmotions sx={{ fontSize: 20 }} />
                  </IconButton>
                  <TextField
                  ...
                  />
                  {newComment[post.id]?.trim() && (
                    <Button onClick={() => handleAddComment(post.id)} sx={{ color: '#0095f6' }}>
                      Post
                    </Button>
                  )}
                </Box>
